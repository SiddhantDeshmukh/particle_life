...
fn init(rng: &mut ThreadRng, window_width: f32, window_height: f32) -> Params {
...
pub fn reset(
    ...
    window_width: f32,
    window_height: f32,
) -> (Params, Vec<Particle>, Vec<Vec<f32>>) {
    // Reinit, redraw
    let params = init(rng, window_width, window_height);
    let particles =
        generate_particles_cm(2000, rng, &colors, 0., X_SIZE, 0., Y_SIZE, 0., 0., 0., 0.);
...
async fn main() {
    ...
    const MAX_MOUSE_PICKUP_RADIUS: f32 = 100.;
    // const SEED: u64 = 420;
    ...
    let colors: Vec<macroquad::color::Color> =
        vec![RED, ORANGE, YELLOW, LIME, PURPLE, BLUE, PINK, SKYBLUE];
    let mut force_matrix = generate_force_matrix(colors.len(), &mut rng);
    let mut particles = generate_particles_cm(
        NUM_PARTICLES,
        &mut rng,
        &colors,
        x_min,
    ...
    let mut mouse_pickup_radius: f32 = 50.;
    ...
    loop {
        ...
        let font_size_medium: f32 = clamp(0.05 * window_height, 10., 32.);
        // Drawing
        ...
        // Pickup radius
        draw_circle_lines(
            render_mouse_position.x,
            render_mouse_position.y,
            mouse_pickup_radius,
            1.,
            GRAY,
        );
        let world_mouse_position =
            win_to_world(mouse_position_v, params.window_width, params.window_height);
        // Mouse events
        // Mouse wheel to change pickup size
        let mouse_wheel_y = mouse_wheel().1;
        if mouse_wheel_y != 0. {
            mouse_pickup_radius = clamp(
                mouse_pickup_radius + (mouse_wheel_y * 25.),
                MIN_MOUSE_PICKUP_RADIUS,
                MAX_MOUSE_PICKUP_RADIUS,
            )
        }
        // Particles
        ...
        if !is_paused {
            ...
            if is_mouse_button_down(MouseButton::Left) {
                ...
                particles = particles
                    .par_iter_mut()
                    .map(|p: &mut Particle| {
                        if mouse_position_v.distance(world_to_win(
                            p.position,
                            params.window_width,
                ...
                .show(egui_ctx, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.button("Reset").clicked() {
                            (params, particles, force_matrix) =
                                reset(&mut rng, &colors, sim_window_width, sim_window_height);
                        }
                    ...
                    ui.horizontal(|ui| {
                        ...
                        slider!(&mut params.max_radius, 1., 50., ui);
                    });
                    ui.horizontal(|ui| {
